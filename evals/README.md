...
| Name | What it checks |
|------|----------------|
| `create-file` | Writes a new file with exact requested contents (`write`) |
| `edit-file` | Reads a buggy file and applies a correct fix (`read` + `edit`) |
| `answer-question` | Inspects a seeded repo and answers a factual question (`read`/`grep`/`glob` + reasoning) |
